"""
================================================================================
COMBINED GUARD RAIL CLOUD ENGINE — SINGLE STAKE MODE
================================================================================
Runs the Combined Guard Rail bot in Single Stake Mode (1 x order) and serves a
live Flask web dashboard for remote monitoring on Railway/Render.
================================================================================
"""
...
# State container for web dashboard
bot_state = {
    "balance": 0.0,
    "stake": 1.0,
    "double_stake": False,
...
DOUBLE_STAKE            = False    # Enforce Single Stake
CONFIDENCE_THRESHOLD    = float(os.getenv("CONFIDENCE_THRESHOLD", "0.85"))
MIN_SAFE_BTC_GAP        = float(os.getenv("MIN_SAFE_BTC_GAP", "15.00"))
T2_MIN_BTC_GAP          = float(os.getenv("T2_MIN_BTC_GAP", "2.00"))
...
DYNAMIC_STAKE_PERCENT   = float(os.getenv("DYNAMIC_STAKE_PERCENT", "0.50"))
STREAK_WIN_CAP          = int(os.getenv("STREAK_WIN_CAP", "4"))


bot_state["double_stake"] = False
...
WS_HEADERS = {
    ...
    "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) CombinedSingleCloudBot/1.0"
}
...
def get_dynamic_starting_stake(client, current_streak):
    global STARTING_STAKE_USD
    if current_streak == 0:
        bal = get_live_balance(client) if client else None
        if bal and bal > 1.00:
            new_stake = max(1.00, round(bal * DYNAMIC_STAKE_PERCENT, 2))
            if new_stake > bal - 0.05:
                new_stake = max(1.00, round(bal - 0.05, 2))
            STARTING_STAKE_USD = new_stake
            log(f"🔄 Dynamic starting stake set to {DYNAMIC_STAKE_PERCENT*100:.0f}% of wallet balance (${bal:.2f} USDC) -> STARTING_STAKE_USD = ${STARTING_STAKE_USD:.2f}")
...
def evaluate_combined_guard_rails(up_ask, dn_ask, last_price, ptb, ws_history, mark, prior_conflicts=0):
    ...
    if target_ask is None:
        return target_side, 0.99, True, f"{target_side} dried up (100% win)"

    if target_ask < CONFIDENCE_THRESHOLD:
        return None, None, False, f"Ask price (${target_ask:.2f}) below confidence threshold (85¢)"

    if target_ask > MAX_ENTRY_PRICE:
        return None, None, False, f"Ask price (${target_ask:.2f}) exceeds maximum cap (95¢)"

    momentum_ok = True
    if len(ws_history) >= 3:
        p1, p2, p3 = ws_history[-3][1], ws_history[-2][1], ws_history[-1][1]
        if btc_dir == "UP" and (p3 < p2 < p1): momentum_ok = False
        elif btc_dir == "DOWN" and (p3 > p2 > p1): momentum_ok = False
    ...
    if mark > 2:
        ...
        # T-2s ONLY: Block if 2+ prior conflicts fired
        if prior_conflicts >= 2:
            return None, None, False, f"T-2s BLOCKED: {prior_conflicts} prior conflicts detected → Skipping (fake-out risk)"
        if btc_gap >= T2_MIN_BTC_GAP:
            return target_side, target_ask, True, f"T-2s CONFIRMED LATE: Gap=${btc_gap:.2f} >= $2.00"
        else:
            return None, None, False, f"T-2s REJECTED: Gap=${btc_gap:.2f} < $2.00 (High flip risk)"


def run_bot_engine():
    log("Starting Combined Guard Rail Bot (SINGLE STAKE) Cloud Engine...")
    client = None
    ...
    stake = get_dynamic_starting_stake(client, 0)
    ...
    while True:
        try:
            ...
            side = price = None
            last_price = ptb
            conflict_count = 0  # tracks consecutive conflict signals
                ...
                for mark in PROBE_MARKS:
                    ...
                    if in_bet and side is None:
                        s, ask, approved, reason = evaluate_combined_guard_rails(
                            up_ask, dn_ask, last_price, ptb, ws.get_recent_ticks(), mark, conflict_count
                        )

                        log(f"T-{mark}s Probe: BTC=${last_price:,.2f} | Guard Rail: {reason}")

                        if not approved or s is None:
                            if "Conflict" in reason:
                                conflict_count += 1
                            if mark == BET_WINDOW_END: bot_state["skipped"] += 1
                            continue
                        ...
                        if client:
                            try:
                                ...
                                if DOUBLE_STAKE:
                                    log(f"[ORDER] LIVE DOUBLE ORDER (FAK): 2 x ${single_stake:.2f} on {side} @ ${price:.4f}")
                                    ...
                                    log(f"[ORDER] LIVE SINGLE ORDER (FAK): ${single_stake:.2f} on {side} @ ${price:.4f}")
                                    client.create_and_post_market_order(MarketOrderArgsV2(token_id=tid, amount=single_stake, price=price, side="BUY", order_type=OrderType.FAK))
                            except Exception as e:
                                log(f"Execution Error: {e}")
            ...
            if side:
                ...
                if win is True:
                    ...
                    # Check if streak cap is reached (Take Profit)
                    if bot_state["streak"] >= STREAK_WIN_CAP:
                        log(f"🏆 STREAK CAP REACHED ({STREAK_WIN_CAP}/{STREAK_WIN_CAP} WINS)! Pocketing profit of ${stake - STARTING_STAKE_USD:.2f}")
                        bot_state["streak"] = 0
                        stake = get_dynamic_starting_stake(client, 0)
