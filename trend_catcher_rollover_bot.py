#!/usr/bin/env python3
"""
trend_catcher_rollover_bot.py — Standalone Trend-Catcher Rollover Bot.
Optimized for peak hours, capturing early breakout trends and rolling over 100% of winnings.

Safety Features:
  1. TREND-BASED MOVE REQUIREMENT:
     - T-80s to T-70s: requires a $40.00+ move from PTB.
     - T-60s to T-35s: requires a $20.00+ move from PTB.
     - T-30s to T-15s: requires a $15.00+ move from PTB.
     - Does NOT trade after T-15s to avoid late-window reversals.
  2. MAX_PRICE_LIMIT: Avoid buying extremely overpriced contracts (default >= $0.90).
...
Winnings Rollover Compounding:
  ...
  2. Streak Rollover: If we win, the payout (stake + profit) becomes the stake
     for the next trade (e.g., $1.00 -> $1.25 -> $1.56...).
...
WAKE_UP_BEFORE      = 90    # wake up early at T-90s to monitor early trends
STARTING_STAKE_USD  = 1.00  # initial stake at the start of a streak

# Bet window: place bet at the FIRST probe within this range
BET_WINDOW_START    = 80    # earliest check (T-80s)
BET_WINDOW_END      = 15    # latest check (T-15s) - avoids close-only trading
            ...
            if remaining <= mark:
                ...
                if in_bet_window and decided_side is None:
                    if last_clear_signal is None:
                        ...
                        # Determine required move based on Trend-Catcher countdown tier
                        if mark >= 70:
                            required_move = 40.0
                            tier_desc = "mega-move"
                        elif 35 <= mark <= 60:
                        ...
                        if sig_ask is not None:
                            # Safety Rule 3: Max contract price cap
...
def main():
    ...
    print(f"\n{'═'*72}")
    print(f"  TREND-CATCHER ROLLOVER BOT (STREAK COMPOUNDING)  +  {mode_str}")
    print(f"{'═'*72}\n")
