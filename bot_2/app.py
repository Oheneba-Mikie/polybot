...
# ==============================================================================
# BOT OPTION B: Buy @ $0.98 -> Hold to $1.00 Resolution (+2.04% Gain per Cycle)
# ==============================================================================

BUY_TARGET_PRICE = 0.98
RESOLUTION_PAYOUT = 1.00
MIN_SHARES = 1
...
bot_state = {
    "name": "Bot Option B (Buy 0.98 -> Hold to 1.00 Resolution)",
    "status": "Initializing",
...
def execute_option_b_trade(side_name, token_id, ask_price, ask_size):
    global cycle_traded
    if cycle_traded:
        return
    ...
    if clob_client and bot_state["mode"] == "LIVE":
        try:
            ...
            if res and (res.get("orderID") or res.get("success")):
                ...
                buy_success = True
                log(f"[SUCCESS] Buy Order Placed & Filled! ID: {order_id}")
            else:
                log(f"[ORDER FAILED] {res}")
        except Exception as e:
            log(f"[ORDER ERROR] {e}")
    else:
        buy_success = True
    ...
    if buy_success:
        ...
        log(f"[HOLDING POSITION] Holding {target_shares} {side_name} into candle resolution for $1.00 payout...")

def resolve_cycle_payout():
    ...
    log(f"[CYCLE RESOLVED] Payout Collected: ${payout:.2f}! Profit: +${profit:.4f} USDC (+2.04%) | New Rollover Balance: ${bot_state['balance']:.2f}")
...
def on_message(ws, message):
    ...
    try:
        ...
        if updated and up_price is not None and down_price is not None:
            ...
            # TRIGGER OPTION B: Guaranteed Final Seconds Window (<= 15s remaining) with Oracle Delta verification
...
def bot_loop():
    ...
    while True:
        try:
            ...
            if slug != active_slug:
                # If we had a position in the previous cycle, resolve it now!
                if bot_state["active_position"]:
                    resolve_cycle_payout()
