...
# Target a rich, fully-settled 5m market window: 7:45PM ET (23:45 UTC -> 1789256700)
# or 7:40PM ET (23:40 UTC -> 1789256400)
w_s = 1789256700
...
print(f"📊 COMPLETE 5-MINUTE CHRONOLOGICAL AUDIT: {slug} ({t_start_utc} to {t_end_utc})", flush=True)
print(f"Total Transactions in Window: {len(parsed)} trades", flush=True)
print("="*150, flush=True)

# Find all simultaneous same-second crosses (Time gap <= 1.5s) across the whole 5 minutes
opps = []
for i, t1 in enumerate(parsed):
    for j in range(i+1, min(i+30, len(parsed))):
        t2 = parsed[j]
        dt = t2["ts"] - t1["ts"]
        if dt > 1.5: break
        ...
        if (t1["outcome"] in ("UP","YES") and t2["outcome"] in ("DOWN","NO")) or (t1["outcome"] in ("DOWN","NO") and t2["outcome"] in ("UP","YES")):
            ...
            if comb <= 0.985:
                ...
                # Dry up time
                subsequent = [t for t in parsed[j+1:] if t["ts"] - t1["ts"] <= 15.0]
                dry_up = dt
