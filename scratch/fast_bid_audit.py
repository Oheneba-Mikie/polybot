...
# Analyze the active candle and last 8 candles
for i in range(0, 8):
    w_s = cur_w_s - (i * 300)
    slug = f"btc-updown-5m-{w_s}"
    try:
        r_evt = requests.get(f"{GAMMA_HOST}/events?slug={slug}", timeout=2).json()
        ...
        r_up = requests.get(f"{CLOB_HOST}/book?token_id={up_id}", timeout=1.5).json()
        r_dn = requests.get(f"{CLOB_HOST}/book?token_id={dn_id}", timeout=1.5).json()
        ...
        for up_b in up_bids[:10]:
            for dn_b in dn_bids[:10]:
