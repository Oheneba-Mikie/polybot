...
print("="*100)
print("🚀 FAST PARALLEL 6-HOUR POLYMARKET AUDIT: BUY 0.97 --> SELL 0.98 / 0.99 SCALPER")
print("="*100)
...
def fetch_candle_data(ts):
    ...
    try:
        ...
        r_t = requests.get(f"{DATA_HOST}/trades?market={cid}&limit=500", timeout=6).json()
...
print(f"Fetching {len(candle_timestamps)} candles in parallel...")
with ThreadPoolExecutor(max_workers=20) as executor:
    results = list(executor.map(fetch_candle_data, candle_timestamps))

trades_by_candle = [r for r in results if r is not None]
trades_by_candle.sort(key=lambda x: x["ts"])
...
def run_scalp_sim(entry_price=0.97, target_price=0.98, starting_balance=5.0, rollover=True):
    ...
    for c in trades_by_candle:
        ...
        # Check both sides for 0.97 entry
        for side_name, side_trades in [("UP", up_trades), ("DOWN", dn_trades)]:
            ...
            for i, (ts, px, sz) in enumerate(side_trades):
                if not entered:
                    if px >= entry_price and px < 0.985:
                        entered = True
                        entry_ts = ts
                else:
                    if px >= target_price:
...
print("="*100)
print(f"📊 BACKTEST RESULTS: $5.00 STARTING BALANCE ACROSS PAST 6 HOURS (72 CANDLES)")
print("="*100)
