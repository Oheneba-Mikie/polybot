#!/usr/bin/env python3
"""
triple_close_scaling_bot.py — Triple Streak Close-Only Compounding Bot

================================================================================
HOW THIS BOT OPERATES:
================================================================================
- Splits your wallet balance into 3 equal parts (minus a 5c fee buffer for each).
- Runs 3 concurrent Close-Only streaks (Streak 1, Streak 2, Streak 3).
- All 3 streaks place trades only at the market close (T-12s to T-5s) based on
  high-probability close signals (price move >= $5.00).
- Since all 3 buy the close, they compound in parallel on the safest signals.
- Each streak has a 4-Win Cap (Take Profit) to bank profits and reset.
- If a streak completes or loses, it resets to 1/3 of the current wallet balance.
...
Step 4: Run the bot:
        python triple_close_scaling_bot.py
...
# Close buying window
BET_WINDOW_START    = 12    # earliest we'll check (T-12s)
...
def run_probe_phase(ws: WSFeed, ptb: float, w_end: int, market: dict, clob_client=None):
    ...
    print(f"\n  Streaming last {WAKE_UP_BEFORE}s — close-buying probes at T-12s to T-5s")
    ...
    while True:
        ...
        for mark in PROBE_MARKS:
            ...
            if remaining <= mark:
                ...
                # Close Bet window (T-12s to T-5s)
                ...
                if in_bet_window and last_clear_signal is not None:
                    ...
                    if abs_move >= required_move and last_clear_signal == current_dir:
                        ...
                        if sig_ask is not None:
                            # Place bets on all 3 streaks in parallel
                            # Streak 1
...
def print_summary(results, w_start, w_end, ptb, last_price, s1_side, s1_price, s2_side, s2_price, s3_side, s3_price):
    ...
    if s3_side:
        print(f"  🎯 Bet [Streak 3] : {s3_side} @ ${s3_price:.4f}")
    print(f"  {'─'*72}")
...
def main():
    ...
    print(f"\n{'═'*72}")
    print(f"  TRIPLE STREAK CLOSE-ONLY BOT  +  {mode_str}")
    print(f"{'═'*72}\n")
    ...
    while True:
        try:
            ...
            print(f"  [S1] Streak 1  : {streak_1_wins}/{STREAK_WIN_CAP} Wins  |  Stake: ${streak_1_stake:.2f} pUSD")
            print(f"  [S2] Streak 2  : {streak_2_wins}/{STREAK_WIN_CAP} Wins  |  Stake: ${streak_2_stake:.2f} pUSD")
            print(f"  [S3] Streak 3  : {streak_3_wins}/{STREAK_WIN_CAP} Wins  |  Stake: ${streak_3_stake:.2f} pUSD")
