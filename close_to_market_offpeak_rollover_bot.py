#!/usr/bin/env python3
"""
close_to_market_offpeak_rollover_bot.py — Off-Peak Close-to-Market Rollover Bot.
Optimized for late-night / off-peak hours when USA is sleeping (thin liquidity, choppy markets).

Safety Features:
  1. MIN_PRICE_MOVE: Only trade if BTC has moved at least $25.00 away from the PTB.
     This avoids risky, low-margin trades during choppy off-peak trading.
  2. MAX_PRICE_LIMIT: Avoid buying contracts priced above $0.75. This guarantees
     at least a 33%+ profit payout on winning trades and avoids bad risk/reward entries.
...
Winnings Rollover Compounding:
  ...
  2. Streak Rollover: If we win, the payout (stake + profit) becomes the stake
     for the next trade (e.g., $1.00 -> $1.33 -> $1.77...).
  ...
  5. Startup Reconstruction: Reconstructs the streak stake from the Polymarket API
     history if live trading is active.
"""
...
WAKE_UP_BEFORE      = 45    # wake up at T-45s (shorter to avoid off-peak drift)
STARTING_STAKE_USD  = 1.00  # initial stake at the start of a streak

# Bet window: place bet at the FIRST probe within this range
BET_WINDOW_START    = 25    # earliest we'll check (T-25s)
BET_WINDOW_END      = 5     # latest we'll bet  (T-5s)

# Strategy Rules:
CONFIDENCE_THRESHOLD = 0.65  # dominant side must be priced at $0.65+
MAX_PRICE_LIMIT      = 0.75  # Off-Peak Price Cap: guarantees 33%+ payout
MIN_PRICE_MOVE       = 25.0  # Off-Peak Move Guard: BTC must move $25.00+ from PTB

# Order-book probe marks (seconds before close)
PROBE_MARKS = [45, 40, 35, 30, 25, 20, 18, 15, 12, 10, 8, 5, 3, 2, 1, 0]
...
def reconstruct_stake_on_startup(clob_client):
    try:
        ...
        if up_won is None:
            ...
            print(f"  ⚠️ Last market unresolved. Using last trade cost: ${cost:.2f} pUSD")
            return max(1.00, round(cost, 2))
        
        outcome_bought = last_trade.get("outcome", "").upper() # "UP" or "DOWN"
        winning_outcome = "UP" if up_won else "DOWN"
        
        if outcome_bought == winning_outcome:
            payout = float(last_trade.get("size", 0))
            print(f"  🏆 Last trade was a WIN! Rolled over payout stake: ${payout:.2f} pUSD")
            return round(payout, 2)
        else:
            print("  ❌ Last trade was a LOSS. Starting stake: $1.00 pUSD")
            return 1.00
    except Exception as e:
        print(f"  ⚠️ Error reconstructing stake from API: {e}. Starting stake: $1.00 pUSD")
        return 1.00
            ...
            if remaining <= mark:
                ...
                if in_bet_window and decided_side is None:
                    if last_clear_signal is None:
                        ...
                        required_move = MIN_PRICE_MOVE

                        # Safety Rule 1: Verify current move size is sufficient
                        abs_move = abs(last_price - ptb)
                        if abs_move < required_move:
                            print(f"  🚫 T-{mark}s: BTC move too small for off-peak ($abs_move:.2f < ${required_move:.2f}) — SKIP BET")
                            continue
                        ...
                        if sig_ask is not None:
                            # Safety Rule 3: Max contract price cap (Strict off-peak $0.75 cap)
                            if sig_ask >= MAX_PRICE_LIMIT:
                                print(f"  🚫 T-{mark}s: Contract price too high (${sig_ask:.4f} >= ${MAX_PRICE_LIMIT:.2f}) — SKIP BET (poor R/R)")
                                continue
...
def main():
    ...
    print(f"\n{'═'*72}")
    print(f"  CLOSE TO MARKET OFF-PEAK ROLLOVER BOT (STREAK COMPOUNDING)  +  {mode_str}")
    print(f"{'═'*72}\n")
    ...
    if POLYMARKET_LIVE_TRADING and clob_client is not None:
        ...
        current_stake = reconstruct_stake_on_startup(clob_client)
    ...
    while True:
        try:
            ...
            if decided_side and entry_price:
                ...
                won = (decided_side == actual_direction)
                stake_old = current_stake
                
                if won:
                    payout = stake_old / entry_price
                ...
                # Run official settlement polling in the background
                threading.Thread(target=settle, args=(slug, decided_side, entry_price, stake_old), daemon=True).start()
            else:
