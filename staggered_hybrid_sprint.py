#!/usr/bin/env python3
"""
staggered_hybrid_sprint.py — Advanced Hybrid Polymarket Bot with Staggered Streaks & $25.00 Close Safety
"""
...
def run_probe_phase(ws: WSFeed, ptb: float, w_end: int, market: dict, clob_client=None, phase=1):
    ...
    print(f"  Current Mode: PHASE {phase} " + ("(Sprint 100% Wallet)" if phase == 1 else "(Safe Compounding Staggered Split)"))
    if phase == 1:
                ...
                if in_bet_window:
                    ...
                    if phase == 1:
                        if s1_decided_side is None and last_clear_signal is not None:
                            ...
                            else:                  required_move = 25.0 # Close Safety raised to $25.00
                    ...
                    # ── Phase 2: Safe Compounding Mode ──
                        ...
                        # Streak 2 (Close-Only) — Only trade if Streak 1 has NOT bet in this window (Staggered Rule)
                        if s2_decided_side is None and (5 <= mark <= 12) and last_clear_signal is not None and s1_decided_side is None:
                            required_move = 25.0 # Close Safety raised to $25.00
                            if abs(last_price - ptb) >= required_move and last_clear_signal == current_dir:
                                sig_ask = up_ask if last_clear_signal == "UP" else down_ask
                                if sig_ask is not None:
...
def main():
    ...
    # Validate if loaded stakes match the live wallet balance (handling top-ups and balance changes on startup)
    if state_loaded and POLYMARKET_LIVE_TRADING and clob_client is not None:
        ...
        if bal is not None:
            ...
            if bal >= 10.00 and (streak_1_stake == 1.00 or bal < total_loaded or bal > total_loaded + 2.00):
                print(f"  💰 Startup balance verification: ${bal:.2f} pUSD. Adjusting compounding stakes to fit...")
                ...
                print(f"  💰 Startup balance verification: ${bal:.2f} pUSD. Adjusting sprint stake to fit...")
