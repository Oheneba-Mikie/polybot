            ...
            if leg1_ask <= MAX_LEG1_PRICE:
                ...
                if ok1 and (st1 in ("matched", "live") or tx1 or oid1):
                    ...
                    while not leg2_filled:
                        ...
                        # 5-SECOND PROFIT BAILOUT RULE:
                        # If after 5 seconds Leg 2 hasn't filled, sell Leg 1 to panic buyers at fill_p1 + $0.01+
                        if time_waiting >= 5.0 or rem_loop <= 10:
                            print("\n")
                            log(f"⏱️ [5-SECOND HEDGE TIMER EXPIRED ({time_waiting:.1f}s)] Leg 2 not filled! Cancelling resting order...")
                            if oid2:
                                try:
                            ...
                            # Fetch top bid from panic buyers for Leg 1
                            best_bid_p, best_bid_sz = fetch_book_best_bid(leg1_token)
                            target_sell_p = round(max(best_bid_p, fill_p1 + 0.01), 2)
                            log(f"🚀 [SELLING TO PANIC BUYERS] Best Book Bid: ${best_bid_p:.2f} | Posting Limit Sell for {active_shares:.0f}sh {leg1_name} @ ${target_sell_p:.2f} (+${target_sell_p - fill_p1:.2f} profit)...")
