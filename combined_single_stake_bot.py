"""
================================================================================
COMBINED GUARD RAIL BOT (SINGLE STAKE)
================================================================================
Strategy Overview:
...
- Fallback Confirmation (T-2s): If T-8s conditions are NOT met (tight gap < $5):
    - Probes through T-7s, T-6s, T-5s, T-4s, T-3s, T-2s.
    - Confirms at T-2s ONLY if BTC has widened the gap (>= $2.00) & orderbook aligns.
    - Otherwise SKIPS the window to preserve capital and prevent close flips!
================================================================================
"""
...
WS_HEADERS = {
    ...
    "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) CombinedBot/1.0"
}
...
class WSFeed:
    def __init__(self):
        ...
        self._history = []  # Keep (ts_ms, price) for momentum check
...
def evaluate_combined_guard_rails(up_ask, dn_ask, last_price, ptb, ws_history, mark):
    """
    Combined Guard Rail Evaluator:
    - Checks Price Gap, Dominant Ask, Direction Alignment, and BTC Momentum.
    """
    ...
    if mark >= 5:  # Early Entry Phase (T-8s to T-5s)
    ...
    else:  # Late Confirmation Phase (T-4s to T-2s)
...
def run_probe_phase(ws, ptb, w_end, market, clob_client=None, stake_usd=1.00):
    ...
    print(f"\n  === COMBINED GUARD RAIL BOT (SINGLE STAKE)  last {WAKE_UP_BEFORE}s ===")
                ...
                if clob_client is not None:
                    ...
                    try:
                        ...
                        resp = clob_client.create_and_post_market_order(
                            MarketOrderArgsV2(token_id=tid, amount=stake_usd, price=price, side="BUY")
                        )
...
def main():
    ...
    print(f"\n{'='*72}")
    print(f"  COMBINED GUARD RAIL BOT (SINGLE STAKE)  |  {mode}")
    ...
    if POLYMARKET_LIVE_TRADING and client:
        ...
        print()
        try:
            u = input(f"  Override starting stake? [{stake:.2f}]: ").strip()
            if u: stake = float(u)
